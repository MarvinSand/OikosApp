...
import { Globe, UserCheck, Home as HomeIcon, Users, X, ChevronDown, Send, MessageCircle, ChevronUp } from 'lucide-react'
...
const VISIBILITY_OPTIONS = [
  ...
  { key: 'specific', label: 'Ausgewählte',         icon: Users },
]
...
function CreatePrayerSheet({ onClose, onCreate }) {
  ...
  const [step, setStep] = useState(1) // 1=visibility, 2=category, 3=details
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    try {
      const vis = visibility === 'community' ? 'public' : (visibility || 'siblings')
      await onCreate({
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4, padding: '0 16px 16px' }}>
          {[1, 2, 3].map(s => (
            <div key={s} style={{ flex: 1, height: 3, borderRadius: 2, backgroundColor: s <= step ? 'var(--color-accent)' : 'var(--color-border)' }} />
          ))}
        </div>
        ...
        {step === 1 && (
          <div style={{ padding: '0 16px 24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {VISIBILITY_OPTIONS.map(o => {
                ...
                return (
                  <button
                    key={o.key}
                    onClick={() => { setVisibility(o.key); if (o.key !== 'community') setStep(2) }}
                    style={{
                  ...
                  >
                    ...
                    <span style={{ fontSize: 14, fontWeight: 600, color: visibility === o.key ? 'var(--color-accent)' : 'var(--color-text)' }}>
                      {o.label}
                    </span>
                  </button>
                )
            ...
            {/* Community-Auswahl wenn nötig */}
            {visibility === 'community' && myCommunities.length > 0 && (
              <div style={{ marginTop: 14 }}>
                <p style={{ margin: '0 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--color-text-secondary)' }}>Community wählen</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {myCommunities.map(c => (
                    <button
                      key={c.id}
                      onClick={() => { setSelectedCommunity(c.id); setStep(2) }}
                      style={{
          ...
          <div style={{ padding: '0 16px 24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {CATEGORIES.map(c => (
                <button
                  key={c.key}
                  onClick={() => { setCategory(c.key); setStep(3) }}
                  style={{
            ...
            <button onClick={() => setStep(1)} style={{ marginTop: 16, fontSize: 13, color: 'var(--color-text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}>
              ← Zurück
            </button>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ padding: '0 16px 24px' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button onClick={() => setStep(2)} style={{ fontSize: 13, color: 'var(--color-text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}>
                ← Zurück
              </button>
              <button
