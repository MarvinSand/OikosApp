-- ============================================================
-- Phase 14: Jüngerschaft Stufen – Vollständiger Inhalt
-- Run this in the Supabase SQL Editor (after phase13)
-- ============================================================

-- 1. Allow 'discipleship' as conversation type
...
-- 2. Stufen-Chat mapping (one conversation per stage)
...
-- 11. Mentor notes (private, mentor-only)
...
-- Authenticated users can read all course content, impulses, calls, stage chats
...
-- Stage chats: insert allowed (for auto-creation)
...
-- Testimonies: own all, public ones readable by all authenticated
...
-- Mentor notes: only the mentor can see/edit
...
-- Placeholder lessons (3 per module) – content_text filled later
DO $$
DECLARE
  mod RECORD;
  lesson_titles TEXT[];
  i INT;
BEGIN
  FOR mod IN SELECT id, stage, sort_order FROM public.course_modules ORDER BY stage, sort_order LOOP
    lesson_titles := ARRAY[
      'Lektion 1: Einführung',
      'Lektion 2: Vertiefung',
      'Lektion 3: Reflexion'
    ];
    FOR i IN 1..3 LOOP
      INSERT INTO public.course_lessons (module_id, title, sort_order)
      VALUES (mod.id, lesson_titles[i], i)
      ON CONFLICT DO NOTHING;
    END LOOP;
  END LOOP;
END $$;

-- Placeholder reflection question per lesson
DO $$
DECLARE
  les RECORD;
BEGIN
  FOR les IN SELECT id FROM public.course_lessons LOOP
    ...
    VALUES (les.id, 'Was hat dich in dieser Lektion besonders angesprochen? Wie möchtest du es in deinen Alltag integrieren?', 'reflection', 1)
    ON CONFLICT DO NOTHING;
  END LOOP;
END $$;

-- Weekly impulse placeholders (current week, all 3 stages)
